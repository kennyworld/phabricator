  ...
  public function renderText() {
    ...
    switch ($action) {
      ...
      case ManiphestAction::ACTION_REASSIGN:
        if ($owner_phid) {
          if ($owner_phid == $actor_phid) {
            $text = "{$author_name} claimed {$task_title} {$task_uri}";
          } else {
            $text = "{$author_name} {$verb} {$task_title} to {$owner_name} {$task_uri}";
          }
        } else {
          $text = "{$author_name} placed {$task_title} up for grabs {$task_uri}";
        }
        break;
      default:
        $text = "{$author_name} {$verb} {$task_title} {$task_uri}";
        break;
    }
